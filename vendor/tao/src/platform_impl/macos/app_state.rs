...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    ...
    unsafe {
      // macOS 26 can report "not on the main thread" here during
      // applicationDidFinishLaunching even though AppKit invoked us on the
      // main application callback. We are inside that callback, so using the
      // unchecked marker avoids an otherwise unrecoverable abort.
